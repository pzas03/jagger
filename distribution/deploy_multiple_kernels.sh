...
for i in 2 3
do
    echo "macys-jagger$i.vm.griddynamics.net : cd $JAGGER_HOME/$DISTRIB; ./start.sh profiles/ci-distributed/environment-kernel.properties"
...
for i in 4 5
do
    echo "macys-jagger$i.vm.griddynamics.net : cd $JAGGER_HOME/$DISTRIB; ./start_agent.sh"
	do_on_vm_daemon $i "cd $JAGGER_HOME/$DISTRIB; ./start_agent.sh -Dchassis.coordination.http.url=http://macys-jagger1.vm.griddynamics.net:8089 $1 $2 $3 $4 $5 $6 $7 $8"
done
...
echo "macys-jagger1.vm.griddynamics.net cd $JAGGER_HOME/$DISTRIB; ./start.sh profiles/ci-distributed/environment-master.properties"
